...
> 文档状态：截至 2026-07-16，第一、第二阶段已经实现并发布；第三阶段仍是候选规划。本文保留产品设计背景，但当前可用能力以 [README](README.md)、[API 契约](docs/api.md)、[管理后台指南](docs/admin.md)和[部署指南](docs/deployment.md)为准。

## 当前实现快照

- 已实现：统一事件 API、客户端 HMAC、路由授权、`firing/resolved` 故障生命周期、去重、静默窗口、持久队列、重试与死信、管理后台、飞书/Telegram/ntfy/SMTP，以及 Docker + 宝塔部署；
...
- 设计边界：只接收 `POST /api/v1/events` 统一格式，不维护第三方监控产品的私有格式解析器。
