...
监控服务调用网关后，网关完成认证、数据校验和持久化入队，立即返回。完整事件接口使用：

```http
HTTP/1.1 202 Accepted
```

轻量通知接口为兼容只接受 HTTP 200 的宝塔自定义消息通道，使用：

```http
HTTP/1.1 200 OK
```

两种成功响应都只表示已经持久化入队，然后由后台任务发送飞书消息。这样即使飞书暂时超时，监控服务也不会被阻塞。
...
路由、严重程度、来源和分钟限额固定在令牌上，状态固定为 `info`。它不会创建或关闭活跃故障；成功入队后返回 `200 OK`，最终渠道结果仍在投递记录中确认。
