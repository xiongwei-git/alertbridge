...
成功持久化入队后返回 `200 OK`，以兼容只把 HTTP 200 视为成功的宝塔自定义消息通道。响应结构与完整事件接口相同，例如：
...
这里的 `200 OK` 表示 AlertBridge 已完成认证、校验和持久化入队，不表示外部通知渠道已经收到消息。`outcome=queued` 时应在后台“投递记录”查看最终结果。完整事件接口仍返回 `202 Accepted`。
