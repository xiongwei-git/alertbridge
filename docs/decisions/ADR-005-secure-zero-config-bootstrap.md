...
Amended 2026-07-16 after production Compose runtime verification.

## Context

Requiring operators to clone the source repository, copy JSON, create several host secret files, align group IDs and preconfigure clients made a small prebuilt image unnecessarily difficult to deploy. The desired flow is one Compose file, one operator-selected administrator password file, and all business configuration after startup. The simplification cannot introduce a shared default password, expose credentials through container environment metadata, make the container root filesystem writable, or place the dynamic-configuration key in the SQLite volume.
...
### Environment-sourced Compose Secret

Rejected after runtime verification. Docker Compose materializes this Secret by copying it into the container, which fails when the service root filesystem is read-only. Disabling `read_only` would weaken the container boundary; a file-backed Secret works with the existing non-root and read-only controls.
...
- A new server needs only `compose.yaml`, `.env`, `secrets/admin_password` and Docker; it does not need the repository or build toolchain.
