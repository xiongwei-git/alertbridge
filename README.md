...
创建仅当前账户可读的 `.env`。管理员密码至少 16 字节，不要使用项目内置的通用密码：
...
镜像直接来自 `ghcr.io/xiongwei-git/alertbridge`。Compose 通过 Secret 文件把引导密码交给容器，不会把密码放进容器环境变量。首次启动时：
...
- `.env` 含引导密码，权限必须为 `0600`，不得提交到 Git；
...
```sh
ALERTBRIDGE_ADMIN_PASSWORD='local-password-at-least-16-bytes' \
